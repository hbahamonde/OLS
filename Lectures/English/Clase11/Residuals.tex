...
Since $\hat y_{i} = \beta_{0}+\beta_{1}\text{Democracy}_{i}$, then it's easy to see that $\epsilon_{i}= y_{i}-\hat y_{i}$. Or in simple, the error ($\epsilon_{i}$) is the difference between what I am predicting ($\hat y_{i}$) and what I am observing ($y_{i}$). It makes complete sense: it quantifies how ``wrong'' my model is.
